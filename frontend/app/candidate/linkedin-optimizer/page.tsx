...
export default function LinkedinOptimizerPage() {
  ...
  return (
    <div className="space-y-6">
          <header className="rounded-3xl border border-border-medium bg-background/50 p-8 backdrop-blur-md">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl font-bold text-foreground tracking-tight flex items-center gap-3">
                  LinkedIn Optimizer <span className="text-xs bg-primary/20 text-primary px-2 py-1 rounded-full uppercase tracking-widest font-bold">Advanced</span>
                </h1>
                <p className="mt-2 text-text-secondary max-w-xl leading-relaxed">
                  Choose your optimization method. We recommend the PDF export for a 100% deep audit of your hidden skills.
                </p>
              </div>
              <Link href="/candidate/dashboard" className="flex items-center gap-2 text-sm text-text-muted hover:text-foreground transition-colors">
          ...
          <div className="grid gap-6 lg:grid-cols-12">
            ...
            <div className="lg:col-span-7 space-y-6">
              <article className="rounded-3xl border border-border-medium bg-background/50 p-6 md:p-8">
                
                {/* Mode Switcher */}
                <div className="flex p-1 bg-surface-1/50 border border-border-medium rounded-2xl mb-8 w-fit">
                  <button 
                    onClick={() => setMode("pdf")}
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${mode === "pdf" ? "bg-primary text-foreground shadow-lg shadow-blue-900/20" : "text-text-muted hover:text-slate-300"}`}
                  >
                    <FiFileText /> PDF Audit
                  </button>
                  <button 
                    onClick={() => setMode("url")}
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${mode === "url" ? "bg-primary text-foreground shadow-lg shadow-blue-900/20" : "text-text-muted hover:text-slate-300"}`}
                  >
                    <FiLink /> Profile URL
                  </button>
                  <button 
                    onClick={() => setMode("text")}
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${mode === "text" ? "bg-primary text-foreground shadow-lg shadow-blue-900/20" : "text-text-muted hover:text-slate-300"}`}
                  >
                    <FiType /> Raw Text
                  </button>
                </div>
                ...
                <div className="space-y-6">
                  <AnimatePresence mode="wait">
                      ...
                      >
                        <div 
                          ...
                          className={`group relative cursor-pointer rounded-3xl border-2 border-dashed transition-all duration-300 p-12 text-center ${
                            pdfFile ? "border-emerald-500/50 bg-emerald-500/5" : "border-border-medium bg-surface-1/30 hover:border-blue-500/50 hover:bg-primary/5"
                          } ${isSubmitting ? "opacity-50 cursor-not-allowed" : ""}`}
                        >
                          <input ref={fileInputRef} type="file" accept=".pdf" className="hidden" onChange={onPdfChange} disabled={isSubmitting} />
                          <FiUploadCloud className={`mx-auto text-5xl mb-4 transition-colors ${pdfFile ? "text-emerald-400" : "text-text-muted group-hover:text-primary"}`} />
                          <h3 className="text-lg font-bold text-foreground">{pdfFile ? pdfFile.name : "Drop LinkedIn PDF here"}</h3>
                          <p className="text-sm text-text-muted mt-1 mb-4">Max 5MB &bull; PDF Format</p>
                          <button className="rounded-full bg-slate-800 px-6 py-2 text-xs font-bold text-foreground hover:bg-slate-700 transition-all">
                            {pdfFile ? "Change File" : "Select File"}
                          </button>
                        </div>
                        <div className="flex items-center gap-3 rounded-2xl bg-primary/5 border border-blue-500/10 p-4 text-xs text-blue-300">
                    ...
                    {mode === "url" && (
                      ...
                      >
                        <div className="space-y-2">
                          <label className="text-sm font-bold text-text-secondary ml-1">LinkedIn Profile URL</label>
                          <div className="relative">
                            <FiLink className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted" />
                            <input 
                              type="url"
                              value={linkedinUrl}
                              onChange={(e) => setLinkedinUrl(e.target.value)}
                              placeholder="https://www.linkedin.com/in/username"
                              className="w-full bg-surface-1/50 border border-border-medium rounded-2xl py-4 pl-12 pr-4 outline-none focus:border-blue-500 transition-all text-foreground"
                            />
                          </div>
                        </div>
                    ...
                    {mode === "text" && (
                      ...
                      >
                        <div className="space-y-2">
                          <label className="text-sm font-bold text-text-secondary ml-1">Paste Profile Text</label>
                          <textarea 
                            value={profileText}
                            onChange={(e) => setProfileText(e.target.value)}
                            placeholder="Copy and paste your 'About' section or 'Experience' here..."
                            rows={8}
                            className="w-full bg-surface-1/50 border border-border-medium rounded-2xl p-4 outline-none focus:border-blue-500 transition-all text-foreground resize-none"
                          />
                        </div>
                  ...
                  <button
                    ...
                    className="w-full relative overflow-hidden rounded-2xl bg-primary py-4 font-bold text-foreground transition-all hover:bg-primary disabled:opacity-50 shadow-lg shadow-blue-900/20"
                  >
            ...
            <div className="lg:col-span-5 space-y-6">
              <aside className="rounded-3xl border border-border-medium bg-background/50 p-6 md:p-8 min-h-[600px] flex flex-col">
                <h2 className="text-xl font-bold text-foreground mb-6">Deep Audit Result</h2>
                ...
                {isSubmitting ? (
                  <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-in fade-in zoom-in duration-300">
                    <div className="relative">
                       ...
                       <div className="absolute inset-0 flex items-center justify-center text-primary text-2xl">{LOADING_STEPS[loadingStep].icon}</div>
                    </div>
                    <div className="text-center space-y-2">
                      <p className="text-foreground font-medium text-lg">{LOADING_STEPS[loadingStep].label}</p>
                      <div className="flex gap-1 justify-center">
                        {LOADING_STEPS.map((_, i) => (
                          <div key={i} className={`h-1 w-8 rounded-full transition-colors duration-500 ${i <= loadingStep ? "bg-primary" : "bg-slate-800"}`} />
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center opacity-50">
                    <div className="w-20 h-20 rounded-full border border-border-medium flex items-center justify-center mb-4"><FiCheckCircle className="text-3xl" /></div>
                    <p className="text-text-secondary max-w-[200px]">Complete the input to generate your performance score.</p>
                  </div>
                ) : (
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    <div className={`rounded-3xl p-8 flex flex-col items-center border border-border-subtle ${getScoreStyles().bg}`}>
                      <div className="relative w-40 h-40">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center leading-none">
                          ...
                          <span className="text-[10px] uppercase font-bold tracking-widest text-text-muted mt-1">Impact Score</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {analysis?.improvedHeadline && (
                      <div className="space-y-3">
                        <h4 className="text-xs font-bold uppercase tracking-widest text-text-muted">AI Suggested Headline</h4>
                        <div className="rounded-2xl border border-blue-500/20 bg-primary/5 p-4 text-sm text-blue-100 italic leading-relaxed">&ldquo;{analysis.improvedHeadline}&rdquo;</div>
                      </div>
                    )}
                          ...
                          <div className="space-y-3">
                             ...
                             <div className="space-y-2">
                                {issues.map((issue, idx) => (
                                  <div key={idx} className="flex gap-3 text-sm p-3 rounded-xl bg-surface-1/50 border border-border-medium text-slate-300">
