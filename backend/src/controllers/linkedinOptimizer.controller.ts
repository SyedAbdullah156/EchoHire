...
const cleanupUploadedFile = async (path?: string) => {
  if (!path) return;
  try {
...
export const analyzeLinkedinPdf = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const pdfParse = (await import("pdf-parse")).default as unknown as (dataBuffer: Buffer) => Promise<{ text?: string }>;
    const parsed = await pdfParse(buffer);
    const extractedRaw = (parsed.text ?? "").replace(/\s+\n/g, "\n").trim();

    if (!extractedRaw || extractedRaw.length < 50) {
      throw new AppError("Could not extract meaningful text from this PDF.", 422);
    }
