...
const companyService = __importStar(require("../services/company.services"));
...
const updateCompany = async (req, res, next) => {
    try {
        ...
        const company = await companyService.updateCompanyService(req.params.id, updateData, req.user._id.toString(), req.user.role);
        res.status(200).json({ success: true, data: company });
    }
    catch (error) {
        next(error);
...
const deleteCompany = async (req, res, next) => {
    try {
        await companyService.deleteCompanyService(req.params.id, req.user._id.toString(), req.user.role);
        res.status(200).json({
            success: true,
